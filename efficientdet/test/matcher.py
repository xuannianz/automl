...
class Match(object):
  ...
  def gather_based_on_match(self, input_tensor, unmatched_value,
    ...
    """
    # input_tensor 是 gt_boxes, (num_gt_boxes, 4)
    # ignored_value 和 unmatched_value 都是 (0, 0, 0, 0)
    # concat 之后变成 (2 + num_gt_boxes, 4)
    input_tensor = tf.concat([tf.stack([ignored_value, unmatched_value]),
                              input_tensor], axis=0)
    # match_result 当中 -2 表示 ignore, -1 表示 negative, 这里加 2 之后, 正好和 input_tensor 的前两个元素匹配上了
