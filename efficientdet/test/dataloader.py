...
from test import tf_example_decoder
...
class InputProcessor(object):
  ...
  def set_scale_factors_to_output_size(self):
    ...
    height = tf.shape(self._image)[0]
    width = tf.shape(self._image)[1]
    max_image_size = tf.cast(tf.maximum(height, width), tf.float32)
    image_scale = tf.cast(self._output_size, tf.float32) / max_image_size
...
class InputReader(object):
  ...
  def __call__(self, params):
    ...
    dataset.apply(_prefetch_dataset)
    if self._is_training:
...
if __name__ == '__main__':
  ...
  params['batch_size'] = 1
  reader = InputReader(file_pattern='/home/adam/.keras/datasets/coco/coco_train*', is_training=False,
                       use_fake_data=False)
  reader(params)
