...
from test import argmax_matcher
from test import box_list
from test import faster_rcnn_box_coder
from test import region_similarity_calculator
from test import target_assigner
...
def _generate_anchor_configs(min_level, max_level, num_scales, aspect_ratios):
  ...
  Args:
      ...
      num_scales: integer number representing intermediate scales added
        ...
        如果 num_scales = 3, 那么 anchor 的 scales 就是 [2^0, 2^(1/3), 2^(2/3]
...
def _generate_anchor_boxes(image_size, anchor_scale, anchor_configs):
  ...
  Args:
    ...
    anchor_scale: float number representing the scale of size of the base
      ...
      默认值是 4, 那么比如 level=3, stride 就等于 8, 4 * 8 = 32, 就是说 level 3 base anchor 的 shape (32, 32)
    anchor_configs: a dictionary with keys as the levels of anchors and
      values as a list of anchor configuration.
  Returns:
  ...
  for _, configs in anchor_configs.items():
    boxes_level = []
    # 一个 level 上 9 中 anchor 的 config
    for config in configs:
      stride, octave_scale, aspect = config
      if image_size % stride != 0:
        raise ValueError("input size must be divided by the stride.")
      base_anchor_size = anchor_scale * stride * 2**octave_scale
      # anchor 宽高的一半, 用于计算 x1, y1, x2, y2
      ...
      xv = xv.reshape(-1)
      yv = yv.reshape(-1)
      # (4, lh * lw), image_size=640 level=3 时, lh=lw=80
      boxes = np.vstack((yv - anchor_size_y_2, xv - anchor_size_x_2,
                         yv + anchor_size_y_2, xv + anchor_size_x_2))
      # (lh * lw, 4), 这个得到是的某一 level, 某一个种 aspect_ratio, scale 的 anchor
      boxes = np.swapaxes(boxes, 0, 1)
      boxes_level.append(np.expand_dims(boxes, axis=1))
    # concat anchors on the same level to the reshape NxAx4
    boxes_level = np.concatenate(boxes_level, axis=1)
    boxes_all.append(boxes_level.reshape([-1, 4]))
  # (N, 4), image_size=640 时, N=(80*80+40*40+20*20+10*10+5*5)*9=76725
...
def _generate_detections(cls_outputs, box_outputs, anchor_boxes, indices,
  ...
  # 改变坐标顺序, (y1, x1, y2, x2) --> (x1, y1, x2, y2)
  boxes = boxes[:, [1, 0, 3, 2]]
  # run class-wise nms
  detections = []
